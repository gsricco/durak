...
refillSocket.onmessage = function(e) {
    ...
    modalRefill = document.getElementById('modalRefill');
    ...
    if (data.hasOwnProperty('request_id') && data.status === 'open') {
        ...
        requestOpened = true;
        divModalQueries = modalRefill.querySelector("div.modal-manual__queries");
        divModalQueries.innerHTML = `#${data.request_id}`;
        divModalTextul = modalRefill.querySelector("div.modal-manual__text>ul");
        ...
        if (dataStatus === 'error') {
            divModalTextul = modalRefill.querySelector("div.modal-manual__text>ul");
            li = document.createElement('li')
            li.innerHTML = 'Произошла ошибка на сервере. Попробуйте создать заявку ещё раз через некоторое время.';
            divModalTextul.appendChild(li);
